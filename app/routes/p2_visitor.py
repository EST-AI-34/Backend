...
from ..db import all_rows, database, idempotent, jsonb, one
...
from ..security import current_visitor, hash_token, random_token
...
def update_preferences(body: VisitorPreferencesPatch, request: Request, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)]):
...
def public_crowd(festival_code: str, request: Request, response: Response, connection: Annotated[Connection, Depends(database)]):
...
def public_businesses(festival_code: str, request: Request, response: Response, connection: Annotated[Connection, Depends(database)], category: str | None = None):
    festival = published_festival(connection, festival_code)
...
def public_coupons(festival_code: str, request: Request, response: Response, connection: Annotated[Connection, Depends(database)]):
    festival = published_festival(connection, festival_code)
...
def my_bookings(request: Request, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)]):
...
def create_booking(session_id: str, body: BookingIn, request: Request, response: Response, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)], idempotency_key: Annotated[str | None, Header(alias="Idempotency-Key")] = None):
    def work():
        session = one(connection, """SELECT ps.*,p.title FROM program_sessions ps JOIN programs p ON p.id=ps.program_id
            WHERE ps.id=%s AND ps.festival_id=%s AND ps.status='OPEN' AND ps.ends_at>now() FOR UPDATE OF ps""", (session_id, visitor["festival_id"]))
        if not session:
            raise not_found("예약 가능한 프로그램 회차를 찾을 수 없습니다.")
        reserved = one(connection, """SELECT coalesce(sum(party_size),0)::int AS count FROM bookings
            WHERE program_session_id=%s AND status IN ('CONFIRMED','CALLED')""", (session_id,))["count"]
        confirmed = session["capacity"] is None or reserved + body.party_size <= session["capacity"]
...
def cancel_booking(booking_id: str, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)]) -> Response:
    booking = one(connection, "SELECT * FROM bookings WHERE id=%s AND visitor_session_id=%s FOR UPDATE", (booking_id, visitor["id"]))
    if not booking:
        raise not_found()
    ...
    if booking["status"] == "CONFIRMED":
        ...
        reserved = one(connection, "SELECT coalesce(sum(party_size),0)::int AS count FROM bookings WHERE program_session_id=%s AND status IN ('CONFIRMED','CALLED')", (booking["program_session_id"],))["count"]
        ...
        if waiting and (session["capacity"] is None or reserved + waiting["party_size"] <= session["capacity"]):
...
def create_course_plan(body: CoursePlanIn, request: Request, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)]):
    ...
    if body.interests:
        clauses.append("p.category=ANY(%s)")
        values.append(body.interests)
    if body.area_id:
        clauses.append("ps.area_id=%s")
    ...
    selected = select_course(sessions, body.duration_min, body.starts_at)
    if not selected:
        raise not_found("조건에 맞는 운영 중 프로그램을 찾을 수 없습니다.")
    plan = one(connection, "INSERT INTO course_plans(visitor_session_id,input_preferences,expected_duration_min) VALUES(%s,%s,%s) RETURNING *", (visitor["id"], jsonb(body.model_dump()), body.duration_min))
    items = []
    for sequence, session in enumerate(selected, 1):
        reason = f"{session['category']} 관심사와 운영 시간을 반영했습니다."
        item = one(connection, """INSERT INTO course_items(course_plan_id,program_session_id,sequence_no,recommendation_reason)
            VALUES(%s,%s,%s,%s) RETURNING *""", (plan["id"], session["id"], sequence, reason))
        items.append({**item, "program": session})
    return success(request, {**plan, "items": items})


@router.get("/visitor/coupons")
def my_coupons(request: Request, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)]):
...
def issue_coupon(coupon_id: str, request: Request, response: Response, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)], idempotency_key: Annotated[str | None, Header(alias="Idempotency-Key")] = None):
    def work():
        coupon = one(connection, """SELECT c.*,fb.festival_id,b.name AS business_name FROM coupons c
        ...
        if not coupon:
            raise not_found("발급 가능한 쿠폰을 찾을 수 없습니다.")
        issued = one(connection, "SELECT count(*)::int AS count FROM coupon_issues WHERE coupon_id=%s", (coupon_id,))["count"]
        if issued >= coupon["issue_limit"]:
            raise conflict("CAPACITY_EXCEEDED", "쿠폰이 모두 발급되었습니다.")
        visitor_issues = one(connection, "SELECT count(*)::int AS count FROM coupon_issues WHERE coupon_id=%s AND visitor_session_id=%s", (coupon_id, visitor["id"]))["count"]
        if visitor_issues >= coupon["per_visitor_limit"]:
        ...
        connection.execute("INSERT INTO business_events(festival_business_id,visitor_session_id,event_type,source) VALUES(%s,%s,'COUPON_ISSUE','COUPON')", (coupon["festival_business_id"], visitor["id"]))
...
def create_reward_event(body: RewardEventIn, request: Request, response: Response, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)], idempotency_key: Annotated[str | None, Header(alias="Idempotency-Key")] = None):
    def work():
        action = one(connection, """SELECT a.*,c.festival_id,c.daily_point_limit FROM reward_actions a JOIN reward_campaigns c ON c.id=a.campaign_id
            WHERE a.id=%s AND c.festival_id=%s AND c.status='ACTIVE' AND c.starts_at<=now() AND c.ends_at>now() FOR UPDATE OF c""", (body.reward_action_id, visitor["festival_id"]))
        if not action:
            raise not_found("참여 가능한 리워드 행동을 찾을 수 없습니다.")
        count = one(connection, "SELECT count(*)::int AS count FROM reward_events WHERE reward_action_id=%s AND visitor_session_id=%s", (body.reward_action_id, visitor["id"]))["count"]
        ...
        ledger = one(connection, "INSERT INTO point_ledger(visitor_session_id,reward_event_id,points_delta,reason) VALUES(%s,%s,%s,%s) RETURNING *", (visitor["id"], event["id"], action["points"], action["action_type"]))
...
def points(request: Request, visitor: Annotated[dict, Depends(current_visitor)], connection: Annotated[Connection, Depends(database)]):
