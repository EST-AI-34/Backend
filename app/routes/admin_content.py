...
def create_item(festival_id:str,body:ContentItemIn,request:Request,_:Annotated[dict,Depends(festival_access)],user:Annotated[dict,Depends(roles("SUPER_ADMIN","FESTIVAL_MANAGER"))],connection:Annotated[Connection,Depends(database)]):
    if bool(body.resource_type)!=bool(body.resource_id):raise bad_request("VALIDATION_ERROR","resourceType과 resourceId는 함께 입력해야 합니다.")
    if body.resource_type=="PROGRAM" and not one(connection,"SELECT 1 FROM programs WHERE id=%s AND festival_id=%s",(body.resource_id,festival_id)):raise bad_request("FESTIVAL_SCOPE_MISMATCH","프로그램이 같은 축제에 속하지 않습니다.")
    row=one(connection,"INSERT INTO content_items(festival_id,content_type,resource_type,resource_id,slug) VALUES(%s,%s,%s,%s,%s) RETURNING *",(festival_id,body.content_type,body.resource_type,body.resource_id,body.slug));return success(request,row)
...
def create_version(festival_id:str,item_id:str,body:ContentVersionIn,request:Request,_:Annotated[dict,Depends(festival_access)],user:Annotated[dict,Depends(roles("SUPER_ADMIN","FESTIVAL_MANAGER","FIELD_OPERATOR"))],connection:Annotated[Connection,Depends(database)]):
    if not one(connection,"SELECT 1 FROM content_items WHERE id=%s AND festival_id=%s",(item_id,festival_id)):raise not_found()
    row=one(connection,"""INSERT INTO content_versions(content_item_id,author_id,version_no,language,body,change_note)
        ...
        WHERE content_item_id=%s AND language=%s RETURNING *""",(item_id,user["id"],body.language,jsonb(body.body),body.change_note,item_id,body.language));return success(request,row)


@router.post("/admin/festivals/{festival_id}/content-versions/{version_id}/submit")
def submit_version(festival_id:str,version_id:str,request:Request,_:Annotated[dict,Depends(festival_access)],user:Annotated[dict,Depends(roles("SUPER_ADMIN","FESTIVAL_MANAGER","FIELD_OPERATOR"))],connection:Annotated[Connection,Depends(database)]):
    ...
    if not row:raise bad_request("INVALID_STATE_TRANSITION","초안 버전만 검수를 요청할 수 있습니다.")
...
def review_version(festival_id:str,version_id:str,body:ReviewIn,request:Request,_:Annotated[dict,Depends(festival_access)],user:Annotated[dict,Depends(roles("SUPER_ADMIN","REVIEWER"))],connection:Annotated[Connection,Depends(database)]):
    ...
    connection.execute("INSERT INTO content_approvals(content_version_id,reviewer_id,decision,comment) VALUES(%s,%s,%s,%s)",(version_id,user["id"],body.decision,body.comment))
    row=one(connection,"UPDATE content_versions SET status=%s WHERE id=%s RETURNING *",(body.decision,version_id))
    audit(connection,festival_id=festival_id,actor_id=str(user["id"]),action=body.decision,resource_type="CONTENT_VERSION",resource_id=version_id,after_data=row,request_id=request.state.request_id)
    return success(request,row)


@router.post("/admin/festivals/{festival_id}/content-items/{item_id}/publish")
def publish_item(festival_id:str,item_id:str,body:PublishContentIn,request:Request,_:Annotated[dict,Depends(festival_access)],user:Annotated[dict,Depends(roles("SUPER_ADMIN","FESTIVAL_MANAGER"))],connection:Annotated[Connection,Depends(database)]):
    found=one(connection,"""SELECT ci.*,cv.status AS version_status,cv.content_item_id FROM content_items ci
    ...
    if str(found["content_item_id"])!=item_id or found["version_status"]!="APPROVED":raise unprocessable("CONTENT_NOT_APPROVED","이 항목의 승인된 버전만 게시할 수 있습니다.")
    ...
    if row["resource_type"]=="PROGRAM":connection.execute("UPDATE programs SET status='PUBLISHED',updated_at=now(),version=version+1 WHERE id=%s AND festival_id=%s",(row["resource_id"],festival_id))
    audit(connection,festival_id=festival_id,actor_id=str(user["id"]),action="PUBLISH",resource_type="CONTENT_ITEM",resource_id=item_id,after_data=row,request_id=request.state.request_id)
    return success(request,row)


@router.post("/admin/festivals/{festival_id}/content-items/{item_id}/unpublish")
def unpublish_item(festival_id:str,item_id:str,request:Request,_:Annotated[dict,Depends(festival_access)],user:Annotated[dict,Depends(roles("SUPER_ADMIN","FESTIVAL_MANAGER"))],connection:Annotated[Connection,Depends(database)]):
    ...
    if not row:raise bad_request("INVALID_STATE_TRANSITION","게시 중인 콘텐츠만 종료할 수 있습니다.")
    if row["resource_type"]=="PROGRAM":connection.execute("UPDATE programs SET status='UNPUBLISHED',updated_at=now(),version=version+1 WHERE id=%s",(row["resource_id"],))
    audit(connection,festival_id=festival_id,actor_id=str(user["id"]),action="UNPUBLISH",resource_type="CONTENT_ITEM",resource_id=item_id,after_data=row,request_id=request.state.request_id)
    return success(request,row)
...
def ai_decision(festival_id:str,review_id:str,body:AIDecisionIn,request:Request,_:Annotated[dict,Depends(festival_access)],user:Annotated[dict,Depends(roles("SUPER_ADMIN","FESTIVAL_MANAGER","REVIEWER"))],connection:Annotated[Connection,Depends(database)]):
    row=one(connection,"""UPDATE ai_message_reports r SET status='CLOSED',decision=%s,reviewer_id=%s,reviewed_at=now()
